...
def create_user():
    ...
    if users.find({"email": data["email"]}).count() == 0:  # Check if user exists with given email.
        ...
        return Response(data['name']+" with the email address :"+data['email']+" has been added to the MongoDB", status=200,mimetype='application/json')
        # Alternatively , if a user exists with this username .
    else:
...
# Check if user applies more that one fields during search (only one available at a time!)
        if "name" in data and "_id" in data and "category" in data:
            return Response("Please search with one of the three options every time at once: name or _id or category",status=400,mimetype="application/json")
...
def create_product():
    ...
    if admin_session_valid(uuid) == True:
        ...
        if not "name" in data or not "price" in data or not "description" in data or not "category" in data or not "stock" in data:
            return Response("Information incomplete", status=500, mimetype="application/json")
        if products.find({'$and': [{"name": data["name"]}, {"price": data["price"]}, {"description":data["description"]},
                                   {"stock":data["stock"]}]}) == 0:
            product = {"name": data["name"], "price":data["price"], "description": data["description"],
                       "category": data['category'], "stock":data['stock']}
            products.insert_one(product)
            return Response("Product was added successfully added to the collection", status=200, mimetype="application/json")
        else :
